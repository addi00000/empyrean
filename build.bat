...
ECHO INSTALLING UPX && curl -L -O https://github.com/upx/upx/releases/download/v3.96/upx-3.96-win64.zip && tar -xf upx-3.96-win64.zip && mv upx-3.96-win64 upx && del upx-3.96-win64.zip
ECHO COMPILING TO EXE && python -m PyInstaller --onefile --noconsole --upx-dir ./upx -i NONE --distpath ./ .\write.py 
ECHO REMOVING BUILD FILES && PYTHON -c "import os;import shutil;exec('''for file in ['build', '__pycache__', 'write.spec', 'write.py', 'upx-3.96-win64.zip', 'upx']:\n    if os.path.exists(file): os.remove(file) if os.path.isfile(file) else shutil.rmtree(file)''')      "
