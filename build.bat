...
ECHO COMPILING TO EXE && python -m PyInstaller --onefile --noconsole --upx-dir ./upx-3.96-win64 -i NONE --distpath ./ .\write.py 
ECHO REMOVING BUILD FILES && PYTHON -c "import os;import shutil;exec('''for file in ['build', '__pycache__', 'write.spec', 'write.py', 'upx-3.96-win64.zip', 'upx-3.96-win64']:\n    if os.path.exists(file): os.remove(file) if os.path.isfile(file) else shutil.rmtree(file)''')      "
